...
from Paper_RL.论文.Paper.Channel_Generate import Channel_Generate
...
for i in range(K):
    for j in range(N):
        h[i][j] = Channel_Generate(1, 1, dis[i])
...
for i in range(K):
    if r[i] > 0:
        E[i] = c_k[i] * (1 - lam[i]) * R[i] * (f_k[i] ** 2) * k + p_max * (lam[i] * R[i] / r[i]) + c_k[i] * lam[i] * R[
            i] * (fkm[i] ** 2) * k_m
    if r[i] == 0:
        E[i] = c_k[i] * R[i] * (f_k[i]) ** 2 * k
    if (c_k[i] * R[i] * (1 - lam[i])) / f_k[i] >= R[i] * lam[i] / r[i] + (c_k[i] + R[i] * lam[i]) / fkm[i]:
        Q[i] = (c_k[i] * R[i] * (1 - lam[i])) / f_k[i]
    else:
        Q[i] = R[i] * lam[i] / r[i] + (c_k[i] * R[i] * lam[i]) / fkm[i]
